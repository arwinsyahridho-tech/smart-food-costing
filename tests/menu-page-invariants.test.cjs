...
assert.match(source, /MAX_MENU_PHOTO_BYTES=8\*1024\*1024/);
assert.match(source, /MAX_SVG_PHOTO_BYTES=1024\*1024/);
assert.match(source, /TARGET_MENU_PHOTO_BYTES=1024\*1024/);
assert.match(source, /MAX_MENU_PHOTO_DIMENSION=1024/);
assert.match(source, /function formatBytes\(bytes\)/);
assert.match(source, /async function resizeImageToBlob\(file/);
assert.match(source, /async function compressImageFile\(file\)/);
assert.match(source, /qualities=useWebP\?\[\.82,\.75,\.68,\.6\]:\[\.78\]/);
assert.match(source, /if\(file\.type==="image\/svg\+xml"\)return file/);
assert.match(source, /pendingMenuPhotoFile=processed/);
assert.match(source, /pendingEditMenuPhotoFile=processed/);
assert.match(source, /contentType:file\.type/);
assert.match(source, /Foto dikompres: \$\{formatBytes\(file\.size\)\} → \$\{formatBytes\(processed\.size\)\}/);
assert.match(source, /PNG, JPG, JPEG, WEBP maksimal 8 MB dan akan dikompres otomatis\. SVG maksimal 1 MB\./);
